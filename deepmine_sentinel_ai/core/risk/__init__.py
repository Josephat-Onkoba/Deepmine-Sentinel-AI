...
class RiskClassificationService:
    ...
    def _build_evaluation_context(self, stope: Stope, impact_score: float) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_events = OperationalEvent.objects.filter(
                stope=stope,
                event_datetime__gte=timezone.now() - timedelta(hours=24)
            ).count()
    ...
    def _hours_since_last_event(self, stope: Stope) -> float:
        ...
        """
        try:
            last_event = OperationalEvent.objects.filter(stope=stope).order_by('-event_datetime').first()
            if last_event:
                return (timezone.now() - last_event.event_datetime).total_seconds() / 3600
